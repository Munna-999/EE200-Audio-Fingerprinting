...
# 1. PAGE SETUP
st.set_page_config(page_title="EE200: Audio Fingerprinting", layout="wide")   # Set the page to wide mode
...
# Custom HTML/SVG Logo 
logo_html = """
<div style="display: flex; align-items: center; justify-content: center; margin-bottom: 20px;">
<div style="border: 2px solid #00d4b2; border-radius: 12px; padding: 10px; margin-right: 15px; display: flex; align-items: center; justify-content: center; width: 50px; height: 50px;">
<svg width="30" height="30" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
<rect x="3" y="8" width="2" height="8" rx="1" fill="#00d4b2" opacity="0.6"/>
<rect x="7" y="5" width="2" height="14" rx="1" fill="#00d4b2"/>
<rect x="11" y="2" width="2" height="20" rx="1" fill="#00d4b2"/>
<rect x="15" y="5" width="2" height="14" rx="1" fill="#00d4b2"/>
<rect x="19" y="8" width="2" height="8" rx="1" fill="#00d4b2" opacity="0.6"/>
</svg>
</div>
<div>
<h1 style="margin: 0; font-size: 2.2rem; font-weight: 700; color: white;">
<span style="color: #00d4b2;">EE200:</span> Audio Fingerprinting
</h1>
<p style="margin: 0; font-size: 0.8rem; letter-spacing: 2px; color: #7f8c8d; text-transform: uppercase;">
Signals, Systems & Networks · PROJECT
</p>
</div>
</div>
"""
...
with tab1:
    st.markdown("""
<div style="text-align: center; background-color: #17263c; border-radius: 8px; padding: 20px; margin-bottom: 25px; border: 1px solid #243b55;">
    <p style="margin: 0; color: #8bb4e7; font-size: 1rem;">
...
with tab2:
    ...
    if audio_to_process is not None:
        ...
        m6.metric("PROCESSING TIME", f"{int((t5-t1)*1000)} ms")
        ...
        if best_song != "none":
            ...
            st.caption(f"The clip was converted into a time-frequency map(left):brighter means louder at that frequency and moment. Fromm that image, only the **{len(peak_t)} most prominent peaks** were kept(right).")
            
            col_a, col_b = st.columns(2)
            with col_a:
                fig1, ax1 = plt.subplots(figsize=(6, 4), dpi=100)
                ax1.pcolormesh(t, f, Sxx_db, shading='gouraud', cmap='magma')
                ax1.set_ylabel('Freq (Hz)', color='#bdc3c7')
            ...
            with col_b:
                ...
                ax2.set_ylabel('Freq (Hz)', color='#bdc3c7')
            ...
            if full_song_points:
                ...
                ax_db.text(winning_offset, ax_db.get_ylim()[1],'', color='#f1c40f', ha='left', va='bottom')
